...
const { statusCode, body, headers } = await request('assignment11server-phi.vercel.app', {
  headers: {
    message: 'hello world!'
  }
})
...
const { headers } = await request('https://assignment11server-phi.vercel.app/foo')
...
const { trailers } = await request('https://assignment11server-phi.vercel.app/foo')
...
const result2 = await request('https://assignment11server-phi.vercel.app/foo')
// Will match and return mocked data
...
const result3 = await request('https://assignment11server-phi.vercel.app/foo')
...
const result = await request('https://assignment11server-phi.vercel.app/foo?foo=bar')
// Will match and return mocked data
```
