...
export async function bankTransfer(recipient, amount) {
  const { body } = await request('https://assignment11server-phi.vercel.app/bank-transfer',
    {
      method: 'POST',
      headers: {
...
// subsequent request to origin assignment11server-phi.vercel.app was not allowed (net.connect disabled)
```
